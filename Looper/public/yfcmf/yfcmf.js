...
$(function () {
    $('body').on('click','.state',function () {
        ...
        $.post($url, {x: val}, function (data) {
            if (data.code==1) {
                if (data.msg == '正常') {
                    var a = '<button class="btn btn-minier btn-danger test">正常</button>';
                    $btn.children('div').html(a).attr('title','正常');
                    return false;
                } else {
                    var b = '<button class="btn btn-minier btn-yellow test">推荐</button>';
                    $btn.children('div').html(b).attr('title','推荐');
                    return false;
                }
            } else {
                layer.alert(data.msg, {icon: 5});
...
$(function () {
	$('body').on('click','.state-btn',function () {
        ...
        $.post($url, {x: val}, function (data) {
            if (data.code==1) {
                if (data.msg == '正常') {
                    var a = '<button class="btn btn-minier btn-danger">正常</button>';
                    $btn.children('div').html(a).attr('title','正常');
                    return false;
                } else {
                    var b = '<button class="btn btn-minier btn-yellow">禁用</button>';
                    $btn.children('div').html(b).attr('title','禁用');
                    return false;
                }
            } else {
                layer.alert(data.msg, {icon: 5});
           ...
           for(i=0;i<$('#file-5')[0].files.length;i++){
               ...
               $.ajax({ //一个Ajax过程
                   ...
                   success: function(data){
                       layer.alert("loop创建成功", {icon: 6}, function (index) {
                           layer.close(index);
                       });
                   }
               });
...
function delall(id, url) {
    $.ajax({ //一个Ajax过程
        type: "post", //以post方式与后台沟通
        url : "/Looper/admin/Music/music_del", //与此php页面沟通
        dataType:'json',//从php返回的值以 JSON方式 解释
        data: 'music_url='+url ,//发给php的数据
