<?php if (!defined('THINK_PATH')) exit(); /*a:6:{s:52:"E:\wamp\www\YFCMF/app/admin\view\news\news_edit.html";i:1492401718;s:49:"E:\wamp\www\YFCMF/app/admin\view\public\base.html";i:1489327653;s:51:"E:\wamp\www\YFCMF/app/admin\view\public\header.html";i:1492401034;s:53:"E:\wamp\www\YFCMF/app/admin\view\public\left_nav.html";i:1490771066;s:53:"E:\wamp\www\YFCMF/app/admin\view\public\head_nav.html";i:1490763638;s:51:"E:\wamp\www\YFCMF/app/admin\view\public\footer.html";i:1492400493;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
			...
			<div class="col-xs-12">
				<form class="form-horizontal ajaxForm2" name="form0" method="post" action="<?php echo url('admin/News/news_runedit'); ?>"  enctype="multipart/form-data">
									...
									if ($input !=""){
										if($input.length<1 || $input.length>20){
											...
											$.ajax({ //一个Ajax过程
												...
												url : "<?php echo url('admin/News/news_flag'); ?>", //与此php页面沟通
												...
												success: function(date){
													...
													for(i=0;i<date.length;i++){
														$str+='<label id="news_flag_'+date[i]["diyflag_name"]+'"><input class="ace ace-checkbox-2" name="news_flag[]" type="checkbox" id="news_flag_va<?php echo $diyflag['diyflag_name']; ?>" value="'+date[i]["diyflag_name"]+'" /><span class="lbl">'+date[i]["diyflag_name"]+'</span></label>';
													}
													$("#auto_flag").html($str);
													alert("创建成功请在上栏选择");
												}
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" name="news_key" id="news_key"  value="<?php echo $news_list['news_key']; ?>"  placeholder="输入Loop关键字，以英文,逗号隔开" class="col-xs-10 col-sm-6" />
						</div>
					</div>
					<div class="space-4"></div>
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							...
							<span class="lbl">&nbsp;&nbsp;<img src="<?php echo get_imgurl($news_list['news_img']); ?>" width="100" height="70" id="img0" ></span>&nbsp;&nbsp;
							<a href="javascript:;" onclick="return backpic('<?php echo get_imgurl($news_list['news_img']); ?>');" title="还原修改前的图片" class="file">
								撤销修改
							</a>
							<span class="lbl">&nbsp;&nbsp;上传前先用PS处理成等比例图片后上传，默认比例100*70、600*420、800*560像素<br /></span>
						</div>
					</div>
					<div class="space-4"></div>
					<!--从专辑中选择歌曲-->
					<div class="form-group" >
						...
						<div class="col-sm-10">
							<select id="ms" multiple="multiple">
								<?php if(is_array($music_list) || $music_list instanceof \think\Collection || $music_list instanceof \think\Paginator): if( count($music_list)==0 ) : echo "" ;else: foreach($music_list as $k=>$music_result): ?>
								<optgroup label="<?php echo $k; ?>">
									<?php if(is_array($music_result) || $music_result instanceof \think\Collection || $music_result instanceof \think\Paginator): if( count($music_result)==0 ) : echo "" ;else: foreach($music_result as $key=>$v): ?>
									<option value="<?php echo $v['id']; ?>"><?php echo $v['filename']; ?></option>
									<?php endforeach; endif; else: echo "" ;endif; ?>
								</optgroup>
								<?php endforeach; endif; else: echo "" ;endif; ?>
							</select>
					...
					<div class="form-group" >
						<div class="col-sm-10">
							<div id="test11" width="500px" style="margin-left: 200px;">
							</div>
						</div>
					</div>
					<div class="space-4"></div>
					...
					<div class="row">
						<div class="col-xs-12">
							<div class="form-group">
								<div class="col-sm-12">
									<ul>
										<?php if(is_array($music_list1) || $music_list1 instanceof \think\Collection || $music_list1 instanceof \think\Paginator): $i = 0; $__LIST__ = $music_list1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
...
<!-- 此页相关插件js -->
	...
	<script>
		$("#file-5").on('change', function(event) {
			var oMyForm = new FormData();
			for(i=0;i<$('#file-5')[0].files.length;i++){
				oMyForm.append("file"+i, $('#file-5')[0].files[i]);
			}
			$.ajax({ //一个Ajax过程
				type: "post", //以post方式与后台沟通
				url : "<?php echo url('admin/News/music_add'); ?>", //与此php页面沟通
				type: 'POST',
				cache: false,
				data: oMyForm,
				processData: false,
		...
		$('#ms').change(function(){
			...
			$.ajax({ //一个Ajax过程
				type: "post", //以post方式与后台沟通
				url : "<?php echo url('admin/News/music_edit'); ?>", //与此php页面沟通
				dataType:'json',//从php返回的值以 JSON方式 解释
				data: 'fileid='+$fileid ,//发给php的数据
				success: function(data){
					$str="";
					$("#test11").html($str);
					$str='<table><tr><td>音乐名称</td><td width="200" align="center">排序</td><td>操作</td></tr>'
					for(i=0;i<data.length;i++){
						$str+='<tr height="40"><td>'+data[i]["filename"]+'</td>';
						$str+='<td width="200" align="center"><input type="hidden" name="order_id" value='+data[i]["order_id"]+'><img src="/YFCMF/public/img/up.png"><img src="/YFCMF/public/img/down.png"></td>';
						$str+='<td><input type="hidden" name="music_id" value='+data[i]["id"]+'><span name="del">删除</span></td></tr>';
					}
					$str+='</table>';
					$("#test11").html($str);
					$("#ms").val("");
					$('[src="/YFCMF/public/img/up.png"]').on('click',(function(){
						$order_id=$(this).prev().val();
						$id=$(this).parents("td").next().children("input").val();
						$.ajax({ //一个Ajax过程
							type: "post", //以post方式与后台沟通
							url: "<?php echo url('admin/News/music_order'); ?>", //与此php页面沟通
							dataType: 'json',//从php返回的值以 JSON方式 解释
					...
					$('[src="/YFCMF/public/img/down.png"]').on('click',(function(){
					...
					$('[name="del"]').on('click',(function(){
						...
						$.ajax({ //一个Ajax过程
							type: "post", //以post方式与后台沟通
							url: "<?php echo url('admin/News/music_listdel'); ?>", //与此php页面沟通
							dataType: 'json',//从php返回的值以 JSON方式 解释
							data: 'id=' + $id,//发给php的数据
							success: function (data) {
								alert("删除成功");
